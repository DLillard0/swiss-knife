...
**【词性/音标】**：{pos（列常见词性；如该词多词性且常见，可用 n./v./adj. 简写）}{若 IPA 可确定则追加 /IPA/；优先使用美式音标（General American）；不确定则不写音标部分}
...
**【原型 & 常见变形】**：{**先找出 selection 的原型**（即词根/最基础词典形式，如 “running”→“run”、“unbelievable”→“believe”、“contradicted”→“contradict”），并给出原型的词性与中文常见义；若 selection 本身已是原型则跳过；再列该原型的最常见派生/变形；每项含“词性/形式 + 中文常见义”；最多 4 项，用 ①②③④；无合适项写“无”}
...
- 不要输出同义词/反义词/词源/长篇用法。
- IPA 优先使用美式音标（General American）；不确定就省略整段音标（不要猜）。
- “原型 & 常见变形”先点明原型（若 selection 非原型），再列最常见派生/变形；不要穷尽，不要超过 4 项。
...
**【原型 & 常见变形】**：原型：get（v. 得到；变得）；① got（v. 过去式/过去分词）：得到 ② gotten（v. 过去分词）：得到 ③ getting（v. 现在分词）：正在得到/变得
...
**【原型 & 常见变形】**：原型：write（v. 写；书写）；① wrote（v. 过去式）：写了 ② writing（v./n.）：写作 ③ writer（n.）：作者
...
**【原型 & 常见变形】**：原型：run（v. 跑；运行；经营）；① running（v./n.）：跑步；运行 ② ran（v. 过去式）：跑了 ③ runner（n.）：跑者
